package scalabilityBeowulf.takka.nqueens
...
case class NQueenMasterStart(n:Int, nodes:Int) extends MasterMsg
case class Search(board:Board, master:ActorRef[MasterMsg])
case class Solutions(boards:List[Board]) extends MasterMsg
case class NSolutions(number:Int) extends MasterMsg
...
class NQueenDFS extends TypedActor[Search] {
  def typedReceive = {
    
    // example I: report all solutions
    case Search(board:Board, master:ActorRef[MasterMsg]) => {
...
class NQueenMaster extends TypedActor[MasterMsg]{
  ...
  def typedReceive = {
    case NQueenMasterStart(n, nodes) =>
      ...
      val plist = (for (i<- 1 to nodes) yield {
        typedContext.actorOf(Props[Search, NQueenDFS], ProcessNamePrefix+i)
      })
...
object NQueenBench extends App{
  ...
  val testActor = system.actorOf(Props[MasterMsg, NQueenMaster], ProcessPathPrefix)
