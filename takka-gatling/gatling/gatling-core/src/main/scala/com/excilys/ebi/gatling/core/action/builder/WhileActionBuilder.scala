 ...
 */
class WhileActionBuilder(condition: Session => Boolean, loopNext: ChainBuilder, next: ActorRef[Session], counterName: String) extends ActionBuilder {
	...
	def withNext(next: ActorRef[Session]) = new WhileActionBuilder(condition, loopNext, next, counterName)

	def build(protocolConfigurationRegistry: ProtocolConfigurationRegistry) = system.actorOf(Props(new WhileAction(condition, (next: ActorRef[Session]) => loopNext.withNext(next).build(protocolConfigurationRegistry), next, counterName)))
}