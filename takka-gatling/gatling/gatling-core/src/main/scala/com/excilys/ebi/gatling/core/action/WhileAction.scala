 ...
 */
class WhileAction(condition: Session => Boolean, loopNext: ActorRef[Session] => ActorRef[Session], next: ActorRef[Session], val counterName: String) extends Action with TimerBasedIterationHandler with CounterBasedIterationHandler with Logging {

	val loopNextAction = loopNext(typedSelf)
