 ...
 */
class ExpPauseActionBuilder(meanDuration: Long, timeUnit: TimeUnit, next: ActorRef[Session]) extends ActionBuilder {
	...
	def withNext(next: ActorRef[Session]) = new ExpPauseActionBuilder(meanDuration, timeUnit, next)
