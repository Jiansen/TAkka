 ...
 */
class HttpRequestActionBuilder(requestName: String, requestBuilder: AbstractHttpRequestBuilder[_], next: ActorRef[Session], checks: List[HttpCheck[_]]) extends ActionBuilder {

	private[gatling] def withNext(next: ActorRef[Session]) = new HttpRequestActionBuilder(requestName, requestBuilder, next, checks)
