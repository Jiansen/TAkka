...
object GatlingAsyncHandler {
	...
	def newHandlerFactory(checks: List[HttpCheck[_]]): HandlerFactory = {
		...
		(requestName: String, actor: ActorRef[HttpEvent]) => new GatlingAsyncHandler(requestName, actor, useBodyParts)
	}
}
 ...
 */
class GatlingAsyncHandler(requestName: String, actor: ActorRef[HttpEvent], useBodyParts: Boolean)
