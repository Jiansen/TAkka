...
import takka.nameserver.NameServer
...
*/
  ...
  def stop_sup(supActor:ActorRef) = {
    display.close()
    sys_event.clear()
    //scheduler.stop()
    system.stop(supActor)
    //Thread.sleep(1000)
    system.shutdown()
  ...
  def top_proc_sup(IFloor:Int, NFloors:Int, NElevs:Int):ActorRef = {
    //system.actorOf(Props(new sim_sup(IFloor, NFloors, NElevs)), "sim_sup") 
    system.actorOf(Props(new sim_sup(IFloor, NFloors, NElevs)))
  }
