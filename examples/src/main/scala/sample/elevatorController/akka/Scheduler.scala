...
case class SetController(ENo:Int, EPid:ActorRef) extends SchedulerMessage
...
case class Approaching(eNo:Int, floor:Floor) extends SchedulerMessage
...
object scheduler {
  ...
  class SchedulerClass extends Actor {
    override def preStart() = {
      scheduler.update_actor(self)
    }
    ...
    def receive = synchronized {
      // repair an elevator by stopping it and let its supervisor restart it        
...
//      sender ! Stop  //BUGGY CODE
  ...
  def update_actor(act:ActorRef) = synchronized {
    pri_scheduler = act
  }
  
  def stop(sch:ActorRef) = synchronized {
  ...
  private def init() = synchronized {
    Elevs = NameServer.get(TSymbol[ActorRef]('elev_sup))  match {
      case None => Nil
...
*/
  def set_controller(ENo:Int, EPid:ActorRef) = synchronized {
    //println("scheduler "+pri_scheduler)
    pri_scheduler match {
      case dead if dead.isTerminated => new Error("Actor scheduler not found")      
      case sch:ActorRef => sch ! SetController(ENo, EPid) 
    }
  } 
  ...
  def approaching(eNo:Int, floor:Floor):Command = synchronized {
    pri_scheduler match {
      ...
      case sch:ActorRef =>
        ...
        val res = sch.ask(Approaching(eNo, floor)).mapTo[Command]
