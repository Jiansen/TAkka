...
//    Ret = gen_event:start_link({local, sys_event}),
  ...
  def controller_started(ENo:Int, EPid:takka.actor.ActorRef[ElevatorMessage]) {notify(ControllerStartedEvent(ENo, EPid)) }
    
  //
  def repair(ENo:Int) {notify (RepairEvent(ENo))}
