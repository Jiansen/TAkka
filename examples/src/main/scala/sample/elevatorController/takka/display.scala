...
//  object display_elev extends SimpleSwingApplication {
      ...
      val EButtonF = new JPanel(null){ // null layout
        ...
        this.setSize(CW, ((NFloors+1)/2)*EBH + EBH +10)
...
*/
      private def draw_buttons(B:Int, E:Int, NFloors:Int, X:Int, Y:Int, EButtonF:JPanel):Unit = {
        if (B>NFloors) {
          // draw the "repair" button
          val button = new JButton("Repair"){
	        this.setLocation(X-EBW, Y + 10)
	        this.setSize(EBW*2, EBH)
	        EButtonF.add(this)
	      }
	      button.addActionListener(new java.awt.event.ActionListener(){
	          override def actionPerformed(e:java.awt.event.ActionEvent) {
	        	  	display.handle_info(('repair, E))
              }
	      })
        }
        else if (B % 2 != 0) {
...
object display extends EventHandler {
  ...
  var ElevGs:List[(Int, ActorRef[ElevatorMessage])] = _
  ...
  def start_e_graphics(pos:Int, elevGs:List[(Int,JPanel)], floors:List[(Floor, Int)]):List[(Int, ActorRef[ElevatorMessage])] = {
    val n = elevGs.length
    var counter = 0
    for (eg<-elevGs) yield NameServer.get(TSymbol[ActorRef[GSupMessage]]('g_sup)) match {
       case None =>
         counter += 1
          //println("g_sup none")
       ...
       case Some(gSup) =>
         counter += 1
         val props = Props[ElevatorMessage](e_graphic.start_link(pos, eg, floors))
//         val res = ask(gSup, props)(200 millis).mapTo[ActorRef[ElevatorMessage]]
         implicit val timeout = akka.util.Timeout(200 millis)
         val res = gSup ? GElevatorProps(props)
