...
import Patterns._//TODO: remove it.  replaced by `?`

class backend extends Actor[BackendMsg]{
  // constants
  val DB = db_list
...
//-record(state, {accounts, blocked = [], alarms = []}).
  ...
  override def preStart() {
    ...
    backend.update_actor(typedSelf)
  }
  ...
  def typedReceive = {
    case request => handle_call(request, state) match {
      case (reply, newstate) =>
        state = newstate
        println((sender, state, reply))
...
object backend {
  ...
  private var pri_actor:ActorRef[BackendMsg] = _
  ...
  def start_link():Actor[BackendMsg] = {
  ...
  def update_actor(actor:ActorRef[BackendMsg]) {
    pri_actor = actor
  }
  ...
  def accountNo(account:Int):Account = {    
    val res = ask(pri_actor, AccountNo(account), timeout).mapTo[Account]
    Await.result(res, 1 second)
  }

  def accountName(name:String):List[Account] = {
    val res = ask(pri_actor, AccountName(name), timeout).mapTo[List[Account]]
    Await.result(res, 1 second)
  }
  
  def accountAll():List[(Int, String)] = {
    val res = ask(pri_actor, AccountAll, timeout).mapTo[List[(Int, String)]]
    Await.result(res, 1 second)
  }
  ...
  def change_pin(accountNo:Int, oldPin:String, newPin:String):Any = {
    val res = ask(pri_actor, ChangePin(accountNo, oldPin, newPin), timeout)//.mapTo[Symbol]
    Await.result(res, 1 second)
  }
  
  def balance(accNo:Int, pin:String):Any = {
    val res = ask(pri_actor, Balance(accNo, pin), timeout)//.mapTo[Int]
    Await.result(res, 1 second)
  }
  
  def transactions(accNo:Int, pin:String):Any = {//:List[Transaction] = {
    val res = ask(pri_actor, Transactions(accNo, pin), timeout)//.mapTo[List[Transaction]]
    Await.result(res, 1 second)
  }

  def withdraw(from:Int, pin:String, amount:Int):Any = {//:Symbol = {
    val res = ask(pri_actor, Withdraw(from, pin, amount), timeout)//.mapTo[Symbol]
    Await.result(res, 1 second)
  }

  def deposit(to:Int, amount:Int):Symbol = {
    val res = ask(pri_actor, Deposit(to, amount), timeout).mapTo[Symbol]
    Await.result(res, 1 second)
  }
  
  def transfer(from:Int, to:Int, pin:String, amount:Int):Any = {//:Symbol = {
    val res = ask(pri_actor, Transfer(from, to, pin, amount), timeout)//.mapTo[Symbol]
    Await.result(res, 1 second)
  }

  def block(accountNo:Int):Symbol = {
    val res = ask(pri_actor, Block(accountNo), timeout).mapTo[Symbol]
    Await.result(res, 1 second)
  }
  
  def eject(accountNo:Int):Symbol = {
    val res = ask(pri_actor, Eject(accountNo), timeout).mapTo[Symbol]
    Await.result(res, 1 second)
  }
}