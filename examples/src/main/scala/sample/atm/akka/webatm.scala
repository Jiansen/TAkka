...
object webatm {
  ...
  //Interface
  def start(name:Symbol, port:Int):ActorRef = {
    /*
    Root = get_root(),
    Conf = [
        {port, Port},
    ...
    {ok, Pid}.
    */
    assert(system.actorFor("/user/webATM").isTerminated, "webATM has been started.")
    pri_actor = system.actorOf(Props(new webATM_Class(name, port)), "webATM")
    make_name(name, port, pri_actor)
    pri_actor
  }
  
  def start_link(name:Symbol, port:Int):Actor = {
  ...
  def stop() {
    val a = system.actorFor("/user/webATM")
    system.stop(a)
    system.shutdown()
  }  
  ...
  def make_name(name:Symbol, port:Int, pid:ActorRef) {
    // try
  ...
  def display(text:String) = {
    pri_actor ! ('display, text)
  }
    
  def append_line(text:String) = {//Not implemented
    pri_actor ! ('append, text)
  }
  
  def highlight(button:String) = {
    pri_actor ! ('hightlight, button)
  }
  ...
  private def lookup_pid(name:Symbol):ActorRef = {
    val tmp = mydb.matchWith('webatm, a => a match { case (sname:Symbol, port:Int, actor:ActorRef) => name == sname })
    tmp match {
      case (_, _, actor:ActorRef)::Nil => actor
      case _ => throw new Error("ATM Internal Error: webatm.scala L103")
    }
  }
  
  private def update_pid(port:Int, pid:ActorRef) = {
    val tmp = mydb.updateWith('webatm, a => a match { 
      case (name:Symbol, sport:Int, actor:ActorRef) => 
...
class webATM_Class(name:Symbol, port:Int) extends Actor{
  def receive = {
    case ('display, text:String) => 
      guiATM.ScreenPanel.screen.text = text
    case ('append, text:String) => //Not implemented
      Unit
    case ('hightlight, button:String) => button match {
  ...
  object guiATM extends SimpleSwingApplication {
    ...
    reactions += {
      case ButtonClicked(b) => b.text match {//User pressed a button
        case "Withdraw Cash" => atm.event('selection,'withdraw)
        case "Balance Enquiry" => atm.event('selection,'balance)
        case "Mini Statement" => atm.event('selection,'statement)
