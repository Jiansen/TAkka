package scalability.takka.mbrot
...
class WorkerSup(timer:BenchTimer, n:Int, np:Int) extends TypedActor[SupMsg]{
...
//    sys.exit()
    ...
    def start_workers(n:Int, np:Int):List[ActorRef[WorkerMsg]] = {
      (for(_<-1 to np) yield {
        typedContext.actorOf(Props[WorkerMsg].withCreator(new Worker(n, typedSelf)))
      }).toList
    }
...
class Worker(n:Int, parent:ActorRef[SupMsg]) extends TypedActor[WorkerMsg] {
  rows(n, n)
