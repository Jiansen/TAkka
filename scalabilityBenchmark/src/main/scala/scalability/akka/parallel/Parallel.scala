package scalability.akka.parallel
...
object OK
object Start
case class Result(pid:ActorRef, r:Boolean)

class NowTime(n:Int, m:Int) extends Actor {  
  ...
  val pids = for (_ <- 1 to m) yield {
    context.actorOf(Props().withCreator(new LoopActor(self, n)))
  }  
  def receive = {
    case Result(_, _) =>
      counter.decrement
...
class LoopActor(master:ActorRef,n:Int) extends Actor {
