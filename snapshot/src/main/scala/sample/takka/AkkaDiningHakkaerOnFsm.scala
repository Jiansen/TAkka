...
//case class TakenBy(hakker: ActorRef[ChopstickMessage])// TODO check this type
case class TakenBy(hakker: akka.actor.ActorRef)// TODO refine this type
...
class Chopstick extends Actor[ChopstickMessage] with FSM[ChopstickState, TakenBy, ChopstickMessage] {
  ...
  startWith(Available, TakenBy(system.deadLetters.untyped_ref))//TODO

  // When a chopstick is available, it can be taken by a some hakker
  when(Available) {
    case Event(Take, _) =>
      goto(Taken) using TakenBy(sender) replying Taken(typedSelf)
  }
  ...
  when(Taken) {
    ...
    case Event(Put, TakenBy(hakker)) if sender == hakker =>
      goto(Available) using TakenBy(system.system.deadLetters)
  }
...
//    println(system.system.deadLetters)
    run
  }
