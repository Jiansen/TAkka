...
trait AskSupport {
  ...
  def ask[M](actorRef: ActorRef[M], message: M)(implicit timeout: Timeout): Future[Any] = actorRef ? message
  ...
  private[pattern] final class AskableActorRef[M](val actorRef: ActorRef[M]) {
    ...
    def ?(message: M)(implicit timeout: Timeout): Future[Any] = ask(message)(timeout)
  }
}
*/